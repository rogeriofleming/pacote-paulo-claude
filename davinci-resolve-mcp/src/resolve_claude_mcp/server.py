...
def _capture_screenshot_macos() -> bytes:
    """Capture Resolve window (or full screen as fallback) via screencapture. PNG bytes."""
...
def _find_resolve_window_windows():
    """Return (rect, hwnd) of the main DaVinci Resolve window, or (None, None).

    rect is (left, top, right, bottom).
    """
    try:
        import win32gui  # type: ignore[import-not-found]  # Windows-only
    except ImportError:
        return None, None
    ...
    def _cb(hwnd, _):
        ...
        if title and "DaVinci Resolve" in title:
            rect = win32gui.GetWindowRect(hwnd)  # (left, top, right, bottom)
            # Skip tiny/degenerate windows (splash, tooltips)
            if (rect[2] - rect[0]) > 200 and (rect[3] - rect[1]) > 200:
                found.append((rect, hwnd))

    win32gui.EnumWindows(_cb, None)
    if not found:
        return None, None
    # Pick the largest matching window (the main app window)
    found.sort(key=lambda rh: (rh[0][2] - rh[0][0]) * (rh[0][3] - rh[0][1]), reverse=True)
    return found[0][0], found[0][1]


def _bring_to_foreground_windows(hwnd) -> None:
    """Restore + raise the given window so the screen grab sees it.

    GPU apps like Resolve can't be captured while occluded (screen-region grab
    returns whatever is on top), so we surface the window first. Windows blocks
    SetForegroundWindow from a process that doesn't own the current foreground,
    so we use the AttachThreadInput trick to temporarily share input state with
    the foreground thread, which lifts that restriction. All best-effort.
    """
    import time
    import ctypes
    ...
    try:
        ...
        time.sleep(0.35)  # let the window repaint before grabbing
    except Exception as e:
        logger.debug("Could not foreground Resolve window: %s", e)


def _capture_screenshot_windows() -> bytes:
    """Capture the Resolve window region (or primary screen fallback) via mss. PNG bytes."""
    try:
        import mss
        import mss.tools
    except ImportError:
        raise RuntimeError(
            "The 'mss' package is required for screenshots on Windows. "
            "Install with: uv pip install mss"
        )

    rect, hwnd = _find_resolve_window_windows()
    if hwnd is not None:
        _bring_to_foreground_windows(hwnd)
        # Rect may have changed after restore (e.g. from minimized) — re-read it.
        rect, hwnd = _find_resolve_window_windows()

    with mss.mss() as sct:
        # Primary monitor bounds, used to clamp the window rect.
        primary = sct.monitors[1]
        if rect is not None:
            left, top, right, bottom = rect
            # Clamp to the primary monitor so mss never grabs out-of-bounds.
            left = max(left, primary["left"])
            top = max(top, primary["top"])
            right = min(right, primary["left"] + primary["width"])
            bottom = min(bottom, primary["top"] + primary["height"])
...
def _capture_screenshot() -> bytes:
    """Capture the Resolve window, dispatching by platform. Returns PNG bytes."""
    import sys
    ...
    raise RuntimeError(
        ...
        "Supported: macOS (screencapture) and Windows (mss)."
    )
...
def transcribe_audio(
    ...
    """
    Transcribe an audio/video file locally (mlx-whisper on macOS, faster-whisper on Windows/Linux).
    Long files are handled automatically so it never times out.
    ...
    try:
        ...
        return (
            f"Transcription backend missing: {e}\n"
            "Or install the extra: pip install 'resolve-claude-mcp[transcription]'"
        )
    except Exception as e:
        return f"Error: {e}"
...
def transcribe_and_add_subtitles(
    ...
    try:
        ...
        return f"Transcription backend missing: {e}"
    except Exception as e:
        return f"Error: {e}"
